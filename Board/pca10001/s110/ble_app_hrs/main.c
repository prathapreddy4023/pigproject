...
#define HEART_RATE_MEAS_INTERVAL             APP_TIMER_TICKS(3000, APP_TIMER_PRESCALER)/**< Heart rate measurement interval (ticks). */
#define SQURE_WAVE_MEAS_INTERVAL             MSEC_TO_UNITS(500, UNIT_1_25_MS)          /**< Heart rate measurement interval (ticks). */
...
static app_timer_id_t                        m_square_wave_timer_id;
...
uint8_t adc_sample[512]={0x00};
uint16_t adc_index=0;
bool ON_Demand_request_data=false;
bool square_flag=false;
void ADC_value(void)
{
    nrf_adc_conversion_event_clean();
		//nrf_adc_start();
  	adc_sample[adc_index]  = nrf_adc_result_get();
		adc_index++;
	  if(adc_index >= 512)
		{
			adc_index=0;
...
void square_wave_timer_start()
{
	uint32_t err_code;
  err_code = app_timer_start(m_square_wave_timer_id, SQURE_WAVE_MEAS_INTERVAL, NULL);
...
static void square_wave_timeout_handler(void * p_context)
{
	square_flag=true;
...
static void timers_init(void)
{
    ...
    APP_ERROR_CHECK(err_code);
		...
		err_code = app_timer_create(&m_square_wave_timer_id,
                                APP_TIMER_MODE_SINGLE_SHOT,
                                square_wave_timeout_handler);
    APP_ERROR_CHECK(err_code);
}
...
static void ble_evt_dispatch(ble_evt_t * p_ble_evt)
{
    ...
    on_ble_evt(p_ble_evt);
		...
		{
				if(p_evt_write->handle==0x0E)
				{
					if(p_evt_write->data[0]==0x31)
					{
						nrf_gpio_cfg_output(22); 
						 nrf_gpio_pin_clear(22);
						ON_Demand_request_data=true;
					}
...
int main(void)
{
    ...
    for (;;)
    {
			 if(ON_Demand_request_data == true)
				{
							...
							for(k=0;k<i;k++)
							{
									err_code = ble_hrs_heart_rate_measurement_send(&m_hrs, 20, &adc_sample[m]);
							...
							err_code = ble_hrs_heart_rate_measurement_send(&m_hrs, j, &adc_sample[m]);
							if (
											(err_code != NRF_SUCCESS)
											&&
											(err_code != NRF_ERROR_INVALID_STATE)
